...
- Resolve prompts through one prompt module that returns LangChain `ChatPromptTemplate` objects from either a production-labelled remote version or an embedded fallback.
...
- [x] When enabled, prompt resolution requests the production-labelled Langfuse version and falls back on missing or unavailable remote content.
...
- [ ] Tests with mocked Langfuse and controlled time verify tracing metadata, fallback behaviour, rollover, retention, and payload exclusion.
- [x] Langfuse prompt failure emits one warning event without logging prompt text, then uses the embedded version.
