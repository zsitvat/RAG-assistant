...
- Keep commuting and mileage formulas inside the calculator module and surface every applied rate, cap, pro-rata operation, toll, or parking addition in the typed breakdown.
...
- [ ] Business mileage distinguishes one-way and round-trip distance, selects the transport rate, and records parking or toll additions as separate calculation lines.
- [ ] Minimum-distance eligibility and transport-mode constraints produce deterministic rule findings.
- [ ] Missing `distance_is_one_way`, commute days, distance, or transport mode is treated as missing information and never inferred by the model.
