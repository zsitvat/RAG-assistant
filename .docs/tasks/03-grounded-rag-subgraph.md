...
- Give the subgraph its own small `RagState` in `src/app/rag/state.py` and compile it independently from the main agent; the seam is question/category in and one typed `RagResult` out.
