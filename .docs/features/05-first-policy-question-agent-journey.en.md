...
### State (`app/agent/state.py`) and domain models (`app/agent/model.py`)
...
### The seven nodes (`app/agent/nodes.py`, `app/agent/graph.py`)
...
### Required slots and clarification (`app/agent/slots.py`)
...
### Tools (`app/agent/calculator.py`, `app/agent/rule_checker.py`, `app/agent/deadline.py`, `app/agent/tools.py`)
...
### Structured output (`app/agent/structured.py`)
...
| File | Responsibility |
| --- | --- |
| `app/agent/model.py` | `Intent`, `Decision`, `ExpenseClaim`, `CalculationResult`, `IntentClassification` |
| `app/agent/state.py` | `AgentState`, loop-budget constants |
| `app/agent/current_request.py` | `CurrentRequest` — latest-request messages, guardrail derivations, and `model_context()` for filtered model input |
| `app/agent/slots.py` | `RequiredSlotTable` |
| `app/agent/structured.py` | `StructuredOutputRunner` |
| `app/agent/prompts.py` | the four embedded prompt templates |
| `app/agent/deadline.py` | `DeadlineChecker` |
| `app/agent/calculator.py` | `ReimbursementCalculator`, `CalculationInputError` |
