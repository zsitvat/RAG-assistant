...
The per-item task is `async def task(*, item, **_): ...` and awaits `AgentService.ainvoke_graph()`
directly — it must be async because `dataset.run_experiment()` runs on its own event loop
...
loop and silently defeat `max_concurrency`. Since `ainvoke_graph()` awaits `graph.ainvoke(...)`
natively, N graph invocations run concurrently against Ollama on that one event loop without any
thread hop. Each task builds a fresh `thread_id` from `{load_run_id}-rep{n}-{item.id}` (unique per
item **and** repetition, so no two measured turns share LangGraph conversation state) and times the
complete `agent_service.ainvoke_graph()` call with `time.monotonic()`; a raised exception is caught
and returned as `{"error": ...}` rather than propagating, so one failed item doesn't drop out of the
aggregate silently.
