...
from app.api.schemas import ChatResponse, EvaluationResponse, LoadTestResult
...
LOAD_TEST_RESULT_FIELDS = {
    "load_run_id",
    "dataset_name",
    "query_count",
    "max_concurrency",
    "total_duration_ms",
    "throughput_queries_per_minute",
    "latency_mean_ms",
    "latency_median_ms",
    "latency_p95_ms",
    "error_count",
...
def test_load_test_result_contract_reports_the_documented_aggregate_fields():
    assert set(LoadTestResult.model_fields) == LOAD_TEST_RESULT_FIELDS
...
async def test_openapi_exposes_chat_and_eval_as_distinct_response_schemas(client):
    ...
    assert "/admin/load-test" in schema["paths"]
    ...
    load_test_response_ref = schema["paths"]["/admin/load-test"]["post"]["responses"]["200"][
        "content"
    ]["application/json"]["schema"]["$ref"]

    assert len({chat_response_ref, eval_response_ref, load_test_response_ref}) == 3
    assert chat_response_ref.endswith("ChatResponse")
    assert eval_response_ref.endswith("EvaluationResponse")
    assert load_test_response_ref.endswith("LoadTestResult")
