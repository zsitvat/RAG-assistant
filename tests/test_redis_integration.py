...
from app.integrations.redis import RedisIndexLifecycle, build_redis_client
...
def _redis_available() -> bool:
    try:
        build_redis_client(TEST_REDIS_URL).ping()
...
def redis_client():
    return build_redis_client(TEST_REDIS_URL)
...
def test_full_ingest_idempotent_rerun_and_dimension_mismatch_rebuild(redis_client, vector_store):
    catalogue = load_rule_catalogue()
    ingestor = PolicyCorpusIngestor()

    first = ingestor.run(redis_client, vector_store, rule_catalogue=catalogue)
    assert first.action == "built"
    assert first.chunk_count > 0
    assert int(redis_client.ft("idx:chunks").info()["num_docs"]) == first.chunk_count

    second = ingestor.run(redis_client, vector_store, rule_catalogue=catalogue)
    assert second.action == "reused"
    assert second.chunk_count == first.chunk_count

    lifecycle = RedisIndexLifecycle(redis_client)
    stale_manifest = lifecycle.read_manifest()
    lifecycle.write_manifest(stale_manifest.model_copy(update={"dimension": 1}))

    third = ingestor.run(redis_client, vector_store, rule_catalogue=catalogue)
    assert third.action == "rebuilt"
    assert third.chunk_count == first.chunk_count
    assert int(redis_client.ft("idx:chunks").info()["num_docs"]) == first.chunk_count


def test_similarity_search_returns_relevant_grounded_chunks(redis_client, vector_store):
    ingestor = PolicyCorpusIngestor()
    ingestor.run(redis_client, vector_store, rule_catalogue=load_rule_catalogue())
...
def test_similarity_search_respects_category_tag_filter(redis_client, vector_store):
    ingestor = PolicyCorpusIngestor()
    ingestor.run(redis_client, vector_store, rule_catalogue=load_rule_catalogue())
...
def test_get_index_stats_reports_total_chunks_and_category_counts(redis_client, vector_store):
    ingestor = PolicyCorpusIngestor()
    ingestor.run(redis_client, vector_store, rule_catalogue=load_rule_catalogue())

    stats = RedisIndexLifecycle(redis_client).get_index_stats()
